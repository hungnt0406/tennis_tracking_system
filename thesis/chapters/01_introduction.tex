...
ordinary phones and consumer cameras, computer vision has become a
...
trajectory. Because a racket hit produces the same trajectory reversal as a
bounce, a simple kinematic rule cannot separate the two, so this is posed as a
learning problem over the trajectory and addressed with gradient-boosted decision
trees \cite{chen2016xgboost} and temporal convolutional networks (TCNs)
\cite{bai2018tcn}.
...
RQ1 (Ball tracking): Among the TrackNet family (V1--V5) and a
...
RQ2 (Court detection): Which backbone offers the best accuracy/latency
...
RQ3 (Bounce detection): For bounce-event timing from a noisy ball
  trajectory, in which a racket hit produces the same kinematic reversal as a
  bounce, how do a gradient-boosted per-frame classifier and a temporal
  convolutional network compare in accuracy, and what does each cost in
  engineering effort?

RQ4 (Integration): Can the strongest model from each sub-task be
...
Chapter~\ref{ch:discussion} discusses cross-cutting findings and threats to
validity, and Chapter~\ref{ch:conclusion} concludes by
