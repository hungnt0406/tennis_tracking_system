...
thematically rather than paper by paper. It starts from the general problem
of localising objects in images, in particular the contrast between
bounding-box and heatmap regression that recurs throughout the thesis, and
then works through the three sub-tasks in turn: ball tracking in sports
video, court and field registration, and event detection from trajectories.
A final pair of sections covers integrated tennis systems and automated
officiating, and draws the preceding material into the research gap that
...
The review points to three gaps. Within each sub-task a wide range of
architectures has been proposed, but they are commonly reported on
heterogeneous datasets and private train/test splits, and most studies
evaluate a single proposed model rather than placing competing designs on a
common footing. This makes like-for-like conclusions about the accuracy,
speed, and robustness trade-offs within a sub-task hard to draw. There is also
little comparison across the tasks: the heatmap formulation that recurs across
ball tracking and court detection, and the temporal and tree-based models that
recur across event detection, are seldom put head to head under a fixed
protocol on the same data. Finally, and to the best of the author's knowledge,
comparatively few works compose the resulting components into a single
end-to-end monocular pipeline whose practical deployability, including model
export and per-frame latency, is assessed, and fewer still close the loop to a
geometric line-call decision from ordinary single-camera video.

This thesis targets those gaps directly. For each of the three sub-tasks it
runs a controlled, within-task comparison under a deterministic evaluation
protocol, then composes the strongest model from each into one reproducible
pipeline that maps detected bounces through an estimated homography to a
qualitative in/out line call. The protocol, datasets, and shared metric
vocabulary behind these comparisons are described next, in
