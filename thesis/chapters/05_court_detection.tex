...
\begin{table}[t]
  ...
  \begin{tabular}{@{}l p{3.5cm} p{3.2cm} r@{}}
    \toprule
    Model & Backbone & Decoder / head & \makecell[r]{Params\\(M)} \\
    \midrule
    TrackNet-court & VGG-style encoder-decoder(from scratch) & transposed-conv decoder with skips & 20.8 \\
    ResNet50       & ResNet-50(ImageNet)                      & three transposed-conv blocks       & 34.0 \\
    HRNet          & HRNet-W32(timm features)                 & stride-4 feature tap + $1\times1$  & 30.9 \\
    MobileNetV3    & MobileNetV3-Small(ImageNet)              & U-Net decoder                       & 1.28 \\
    \bottomrule
  \end{tabular}
\end{table}
...
is also the most accurate overall. It attains the best accuracy
at all four tolerances, including the most demanding five-pixel and seven-pixel
thresholds (0.974 and 0.992), the lowest mean keypoint error (2.46 pixels), and the
joint-best homography success rate (0.999), while carrying 1.28 million parameters against the twenty to
thirty-four million of the others. Its median keypoint error is 1.45 pixels,
which shows that most of its detections are localised to within a pixel or
...
TrackNet-court is slightly better at 7.47 centimetres against 7.67, a
...
relative to the scale of the court, so it is reliable enough to drive the homography that the integrated system of
...
therefore be no more precise than its cell. A five-pixel tolerance is smaller than the eight-pixel
...
and seven pixels; once the tolerance widens, at ten and
...
they good enough to meet the accuracy this task requires. The mean keypoint errors tell the same story
