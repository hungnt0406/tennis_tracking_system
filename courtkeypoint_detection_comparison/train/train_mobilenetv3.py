"""
Train MobileNetV3SmallPose for court keypoint detection.

Usage:
    python -m train.train_mobilenetv3 [--max_samples N] [--epochs N] [--batch_size N] [--device DEVICE]
"""
...
from train.config import MOBILENETV3
...
def train(args):
    ...
    model = MobileNetV3SmallPose().to(device)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if val_pck > best_pck:
            best_pck = val_pck
            patience_counter = 0
            ckpt = CHECKPOINT_DIR / "mobilenetv3_best.pt"
            torch.save(
